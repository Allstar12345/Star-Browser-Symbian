...
AppPage{
    ...
    Component.onCompleted: {show();DBcore.openDB();toolbar.appear();listappear.start();}
    ...
    ToolButtonSense{anchors.right: parent.right;imgsource: "img/save.svg"
        onClicked: {var itemsave = DBcore.readBookmarkItem(bmView.currentItem.myData.id);
            itemsave.title = title.text;
            itemsave.url = url.text;
            DBcore.updateBookmark(itemsave);
           updateList();
            banner.showText("Bookmark Updated");
            close();}
     }
     }
    ...
    anchors{right:parent.right;left:parent.left; top:sectionHeading.bottom; bottom:root.bottom}
