...
AppPage{
    ...
    id: page
    pageId: "6"
    property string urlerr6: ""
    property bool wmenuOpen: false
    property string geoLocationStateW: ""
    signal loadFinishedMenuWindow
    property bool https:false
    property string urlStringw6:appsettings.getSystemSetting("StartRestore", "") == "True" ? appsettings.getSystemSetting("StartURL", "") : appsettings.getSystemSetting("homepage", "") ;
    ...
    Component.onDestruction: console.log("Window 6 Destroyed");
    Component.onCompleted: {show(); webViewHidden=true; windowserver.w6open=true;}
Connections{target: windowserver
    onOpenw6:{show(); webView.visible=true;  webViewHidden=true}
    onHide:{hide();webView.visible=false;  webViewHidden=false}
    onClosew6:{close(); webViewHidden=false; windowserver.w6open=false; windowserver.openWindows-=1;appsettings.saveSystemSetting("externalaccessbool", "true");}
    onCloseAll:{close(); webViewHidden=false; windowserver.w6open=false}
    onReloadAll:{webView.reload.trigger();}
    onReloadW6:{webView.reload.trigger();}
     onOpenLinkW6:{webView.url=url}
...
Flickable{
    ...
    PinchArea {
       ...
       WebViewCustom {
           ...
           url: fixUrl(urlStringw6=== ""? globalHomepage: urlStringw6)
           focus: true
           ...
           onImageRequested:{}
           onVideoRequested:{}
           onTitleChanged: {windowserver.w6Title= webView.title}
           ...
           onUrlChanged: {
               urlStringw6= webView.url
               webView.focus = true;
               windowserver.w6Url=webView.url;

               if(urlStringw6.indexOf("https://") > -1) {console.log("HTTPS"); https=true}
               else{https=false}
           }
           ...
           onFocusChanged: {
            ...
            onLoadStarted: {
                  windowserver.w6Loading=true;
                urlerr6=webView.url;
                progressarea.open()
                fixbackground()
                if(blockedmanager.checkURL(urlerr6)===true){ openPageBlocked();stopblockeddelay.start();}
                else{}
            }
            Timer{id: stopblockeddelay; running: false; interval:200; onTriggered: {webView.stop.trigger();}}

            onLoadFailed:{
                  windowserver.w6Loading=false;
               progressarea.close();
               fixbackground();
               console.log("Page Loading Failed")
                if(wmenuOpen===false)pageSnapper.snapPage(6);
