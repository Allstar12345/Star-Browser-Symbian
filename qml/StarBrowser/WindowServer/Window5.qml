...
AppPage{
    ...
    id: page
    pageId: "5"
    property string urlerr5: ""
    property string geoLocationStateW: ""
    property bool wmenuOpen: false
    signal loadFinishedMenuWindow
    property bool https:false
    property string urlStringw5:appsettings.getSystemSetting("StartRestore", "") == "True" ? appsettings.getSystemSetting("StartURL", "") : appsettings.getSystemSetting("homepage", "") ;
    ...
    Component.onDestruction: console.log("Window 5 Destroyed");
    Component.onCompleted: {show(); webViewHidden=true; windowserver.w5open=true;}
Connections{target: windowserver
    onOpenw5:{show(); webView.visible=true;  webViewHidden=true}
    onHide:{hide();webView.visible=false;  webViewHidden=false}
    onClosew5:{close(); webViewHidden=false; windowserver.w5open=false; windowserver.openWindows-=1;appsettings.saveSystemSetting("externalaccessbool", "true");}
    onCloseAll:{close(); webViewHidden=false; windowserver.w5open=false}
    onReloadAll:{webView.reload.trigger();}
    onReloadW5:{webView.reload.trigger();}
     onOpenLinkW5:{webView.url=url}
...
Flickable{
    ...
    PinchArea {
       ...
       WebViewCustom {
           ...
           url: fixUrl(urlStringw5=== ""? globalHomepage: urlStringw5)
           focus: true
           ...
           onImageRequested:{}
           onVideoRequested:{}
           onTitleChanged: {windowserver.w5Title= webView.title}
           ...
           onUrlChanged: {
               urlStringw5= webView.url
               webView.focus = true;
               windowserver.w5Url=webView.url;

               if(urlStringw5.indexOf("https://") > -1) {console.log("HTTPS"); https=true}
               else{https=false}
           }
           ...
           onFocusChanged: {
            ...
            onLoadStarted: {
                  windowserver.w5Loading=true;
                urlerr5=webView.url;
                progressarea.open()
                fixbackground()
                if(blockedmanager.checkURL(urlerr5)===true){ openPageBlocked();stopblockeddelay.start();}
                else{}
            }
            Timer{id: stopblockeddelay; running: false; interval:200; onTriggered: {webView.stop.trigger();}}

            onLoadFailed:{
                  windowserver.w5Loading=false;
               progressarea.close();
               fixbackground();
               console.log("Page Loading Failed")
                if(wmenuOpen===false)pageSnapper.snapPage(5);
