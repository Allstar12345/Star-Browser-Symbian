...
AppPage{
    ...
    property string urlerr1: ""
    property bool wmenuOpen: false
    signal loadFinishedMenuWindow
    property bool https:false
    property string urlStringw1:appsettings.getSystemSetting("StartRestore", "") == "True" ? appsettings.getSystemSetting("StartURL", "") : appsettings.getSystemSetting("homepage", "") ;
    QtObject { id: create; property Component com: null;function createComponent (qmlfile){
            com = Qt.createComponent(qmlfile)
            com.createObject(page)} }
    MouseArea{anchors.fill: parent; enabled:true}
    Component.onDestruction: console.log("Window 1 Destroyed");
    Component.onCompleted: {show(); webViewHidden=true; windowserver.w1open=true;}
Connections{target: windowserver
    onOpenw1:{show(); webView.visible=true;  webViewHidden=true}
    onHide:{hide();webView.visible=false;  webViewHidden=false}
    onClosew1:{close(); webViewHidden=false; windowserver.w1open=false; windowserver.openWindows-=1;appsettings.saveSystemSetting("externalaccessbool", "true");}
    onCloseAll:{close(); webViewHidden=false; windowserver.w1open=false}
    onReloadAll:{webView.reload.trigger();}
    onReloadW1:{webView.reload.trigger();}
    onOpenLinkW1:{webView.url=url}
}
   ...
   anchors.bottom: wmenuOpen?  parent.bottom:popupbut.top
    ...
    PinchArea {
       ...
       WebViewCustom {
           ...
           url: fixUrl(urlStringw1=== ""? globalHomepage: urlStringw1)
           focus: true
           ...
           onImageRequested:{}
           onVideoRequested:{}
           onTitleChanged: {windowserver.w1Title= webView.title}
           ...
           onUrlChanged: {
               ...
               urlStringw1= webView.url;
               webView.focus = true;
                windowserver.w1Url=webView.url;
               if(urlStringw1.indexOf("https://") > -1) {console.log("HTTPS"); https=true}
               else{https=false}
           }
           ...
           onFocusChanged: {
            ...
            onLoadStarted: {
                  windowserver.w1Loading=true;
                urlerr1=webView.url;
                progressarea.open()
                fixbackground()
                if(blockedmanager.checkURL(urlerr1)===true){ openPageBlocked();stopblockeddelay.start();}
                else{}
            }
            Timer{id: stopblockeddelay; running: false; interval:200; onTriggered: {webView.stop.trigger();}}

            onLoadFailed:{
                  windowserver.w1Loading=false;
               progressarea.close();
               fixbackground();
               console.log("Page Loading Failed")
                if(wmenuOpen===false)pageSnapper.snapPage(1);
