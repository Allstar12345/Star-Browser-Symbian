...
AppPage{
    ...
    id: page
    pageId: "4"
    property string urlerr4: ""
    property string geoLocationStateW: ""
    property bool wmenuOpen: false
    signal loadFinishedMenuWindow
    property bool https:false
    property string urlStringw4:appsettings.getSystemSetting("StartRestore", "") == "True" ?appsettings.getSystemSetting("StartURL", "") : appsettings.getSystemSetting("homepage", "") ;
    ...
    Component.onDestruction: console.log("Window 4 Destroyed");
    Component.onCompleted: {show(); webViewHidden=true; windowserver.w4open=true;}
Connections{target: windowserver
    onOpenw4:{show(); webView.visible=true;  webViewHidden=true}
    onHide:{hide();webView.visible=false;  webViewHidden=false}
    onClosew4:{close(); webViewHidden=false; windowserver.w4open=false; windowserver.openWindows-=1;appsettings.saveSystemSetting("externalaccessbool", "true");}
    onCloseAll:{close(); webViewHidden=false; windowserver.w4open=false}
    onReloadAll:{webView.reload.trigger();}
    onReloadW4:{webView.reload.trigger();}
     onOpenLinkW4:{webView.url=url}
...
Flickable{
    ...
    PinchArea {
       ...
       WebViewCustom {
           ...
           url: fixUrl(urlStringw4=== ""? globalHomepage: urlStringw4)
           focus: true
           ...
           onImageRequested:{}
           onVideoRequested:{}
           onTitleChanged: {windowserver.w4Title= webView.title}
           ...
           onUrlChanged: {
               urlStringw4= webView.url
               webView.focus = true;
               windowserver.w4Url=webView.url;

               if(urlStringw4.indexOf("https://") > -1) {console.log("HTTPS"); https=true}
               else{https=false}
           }
           ...
           onFocusChanged: {
            ...
            onLoadStarted: {
                  windowserver.w4Loading=true;
                urlerr4=webView.url;
                progressarea.open()
                fixbackground()
                if(blockedmanager.checkURL(urlerr4)===true){ openPageBlocked();stopblockeddelay.start();}
                else{}
            }
            Timer{id: stopblockeddelay; running: false; interval:200; onTriggered: {webView.stop.trigger();}}

            onLoadFailed:{
                  windowserver.w4Loading=false;
               progressarea.close();
               fixbackground();
               console.log("Page Loading Failed")
                if(wmenuOpen===false)pageSnapper.snapPage(4);
