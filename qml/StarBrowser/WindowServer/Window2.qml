...
AppPage{
    ...
    property string urlerr2: ""
    property bool wmenuOpen: false
    signal loadFinishedMenuWindow
    property bool https:false
    property string urlStringw2:appsettings.getSystemSetting("StartRestore", "") == "True" ? appsettings.getSystemSetting("StartURL", "") : appsettings.getSystemSetting("homepage", "") ;
    ...
    Component.onDestruction: console.log("Window 2 Destroyed");
    Component.onCompleted: {show(); webViewHidden=true; windowserver.w2open=true;}
Connections{target: windowserver
    onOpenw2:{show(); webView.visible=true;  webViewHidden=true}
    onHide:{hide();webView.visible=false;  webViewHidden=false}
    onClosew2:{close(); webViewHidden=false; windowserver.w2open=false; windowserver.openWindows-=1;appsettings.saveSystemSetting("externalaccessbool", "true");}
    onCloseAll:{close(); webViewHidden=false; windowserver.w2open=false}
    onReloadAll:{webView.reload.trigger();}
    onReloadW2:{webView.reload.trigger();}
     onOpenLinkW2:{webView.url=url}
...
Flickable{
    ...
    PinchArea {
       ...
       WebViewCustom {
           ...
           url: fixUrl(urlStringw2=== ""? globalHomepage: urlStringw2)
           focus: true
           ...
           onImageRequested:{}
           onVideoRequested:{}
           onTitleChanged: {windowserver.w2Title= webView.title}
           ...
           onUrlChanged: {
               urlStringw2= webView.url
               webView.focus = true;
               windowserver.w2Url=webView.url;
               if(urlStringw2.indexOf("https://") > -1) {console.log("HTTPS"); https=true}
               else{https=false}
           }
           ...
           onFocusChanged: {
            ...
            onLoadStarted: {
                  windowserver.w2Loading=true;
                urlerr2=webView.url;
                progressarea.open()
                fixbackground()
                if(blockedmanager.checkURL(urlerr2)===true){ openPageBlocked();stopblockeddelay.start();}
                else{}
            }
            Timer{id: stopblockeddelay; running: false; interval:200; onTriggered: {webView.stop.trigger();}}

            onLoadFailed:{
                  windowserver.w2Loading=false;
               progressarea.close();
               fixbackground();
               console.log("Page Loading Failed")
                if(wmenuOpen===false)pageSnapper.snapPage(2);
           ...
           onLoadFinished: {
               ...
               progressarea.close()
               fixbackground()
               Sappsettings.saveSystemSetting("lasturl", webView.url)
