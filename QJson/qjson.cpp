...
{
   // TRAP_IGNORE(CAknDiscreetPopup::ShowGlobalPopupL((_L("Parse 3")), (_L("stage 3")),KAknsIIDNone, KNullDesC));
...
QVariant QJson::parse(const QByteArray& jsonString, bool* ok) {
 ...
 // TRAP_IGNORE(CAknDiscreetPopup::ShowGlobalPopupL((_L("Parse 2")), (_L("stage 2")),KAknsIIDNone, KNullDesC));
...
QVariant QJson::parse (const QUrl url, bool* ok)
{
    ...
    if( reply->error() == QNetworkReply::NoError )  {
        ...
        int statusCode = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
        QString statusText = reply->errorString();
        emit failure(statusCode, statusText);
